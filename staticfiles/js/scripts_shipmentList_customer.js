document.addEventListener('DOMContentLoaded', () => {
    ...
    if (selectAllDataBtn) {
        selectAllDataBtn.addEventListener('click', () => {
            ...
            updateSelectionOnServer(action, {})
            .then(data => {
    ...
    if (pageSelect) {
        pageSelect.addEventListener('change', function() {
            ...
            if (form) {
                // Remove previously added preserved inputs
                form.querySelectorAll('.preserved-get-param').forEach(el => el.remove());

                // Preserve existing GET params (except 'page') so filters persist across pages
                ...
                for (const [key, value] of params.entries()) {
                    ...
                    input.name = key;
                    input.value = value;
                    input.className = 'preserved-get-param';
