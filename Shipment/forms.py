...
class ShipmentRegistrationForm(forms.ModelForm):
    ...
    class Meta:
        model = Shipment
        exclude = ['shipment_id']
        fields = ('company', 'vessel', 'docs', 'Order_No', 'supplier', 'quanty', 'unit',
    ...
    def clean_shipment_id(self):
        return self.instance.shipment_id  # Ignore incoming value
#=================================================================================#
...
class PendingShipmentForm(forms.ModelForm):
    class Meta:
        model = PendingShipment
        fields = ['company', 'vessel', 'supplier', 'quanty', 'weight', 'size']
        widgets = {
            'company': forms.TextInput(attrs={'list': 'prelistCompanies', 'autocomplete': 'off'}),
            'vessel': forms.TextInput(attrs={'list': 'prelistVessels', 'autocomplete': 'off'}),
            'supplier': forms.TextInput(attrs={'list': 'prelistSuppliers', 'autocomplete': 'off'}),
            'quanty': forms.TextInput(attrs={'placeholder': 'Quantity'}),
            'weight': forms.TextInput(attrs={'placeholder': 'Weight'}),
            'size': forms.Textarea(attrs={'placeholder': 'Size', 'rows': 2}),
        }